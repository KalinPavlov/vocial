defmodule VocialWeb.PollControllerTest do
  ...
  test "GET /polls/new without a logged in user", %{conn: conn} do
    ...
    assert get_flash(conn, :error) == "You must be logged in to access this resource!"
  end
  ...
  defp login(conn, user) do
    conn
    |> post("/sessions", %{session: %{username: user.username, password: user.password}})
  end
end
